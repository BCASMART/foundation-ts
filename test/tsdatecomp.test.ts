...
export const dateCompGroups = [
    TSTest.group("Testing duration functions", async (group) => {
        ...
        group.unary('verifying $durationcomponents()', async (t) => {
            t.expect(Z).toBeDefined() ;
            t.expect(Z.days).toBe(0) ;
            t.expect(Z.hours).toBe(0) ;
            t.expect(Z.minutes).toBe(0) ;
            t.expect(Z.seconds).toBe(0) ;
    
            const n = $duration(A) ;
            const N = $durationcomponents(n) ;
            t.expect(N).toBeDefined() ;
            t.expect(N.days).toBe(3) ;
            t.expect(N.hours).toBe(2) ;
            t.expect(N.minutes).toBe(25) ;
            t.expect(N.seconds).toBe(10) ;
        }) ;
        ...
        group.unary('verifying standard format', async (t) => {
            t.expect($duration2String(A)).toBe("3-02:25:10") ;
            t.expect($duration2String(B)).toBe("02:25:10") ;
            t.expect($duration2String(D)).toBe("02:25") ;
            t.expect($duration2String(E)).toBe("02:00") ;
            t.expect($duration2String(F)).toBe("3-00:00") ;
            t.expect($duration2String(G)).toBe("00:25") ;
            t.expect($duration2String(H)).toBe("00:00:10") ;
            t.expect($duration2String(Z)).toBe("00:00") ;
        }) ;
        ...
        group.unary(`Verifying complex format "${F1}"`, async (t) => {
            ...
            t.expect($duration2String(A, F1)).toBe("3 jours, 2 heures, 25 minutes et 10 secondes") ;
            t.expect($duration2String(B, F1)).toBe("2 heures, 25 minutes et 10 secondes") ;
            t.expect($duration2String(C, F1)).toBe("3 jours, 2 heures et 25 minutes") ;
            t.expect($duration2String(D, F1)).toBe("2 heures et 25 minutes") ;
            t.expect($duration2String(E, F1)).toBe("2 heures") ;
            t.expect($duration2String(F, F1)).toBe("3 jours") ;
            t.expect($duration2String(G, F1)).toBe("25 minutes") ; 
            t.expect($duration2String(H, F1)).toBe("0 minutes et 10 secondes") ;
            t.expect($duration2String(Z, F1)).toBe("") ; 
        }) ;
    }),
    TSTest.group("Testing complex iso string output function", async (group) => {
        ...
        group.unary(`Verifying milliseconds output"`, async (t) => {
            ...
            t.expect(s.slice(p)).toBe('.000Z') ;
            t.expect(s2.slice(p)).toBe('.000+00:00') ;
        }) ;
    })
] ;
